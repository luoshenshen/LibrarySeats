...
def run(cookie, floor, k, flag):
    ...
    result = function.session_get(browser_tools.prereserve_url,
                                  browser_tools.get_prereserve_header(cookie, time.time()))
    if nick in result.text or "预约明天的座位" in result.text:
        return function.fecth(cookie, floor, k, flag, nick)
    else:
        return "进入来选座系统失败，Cookie不正确"
