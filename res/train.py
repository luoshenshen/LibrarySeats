...
if __name__ == '__main__':
    ...
    train_dataset = datasets.ImageFolder("./dataset/training", transform=data_transform["train"])  # 训练集数据
    train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=BATCH_SIZE, shuffle=True,
                                               num_workers=8)  # 加载数据
    len_train = len(train_dataset)
    val_dataset = datasets.ImageFolder("./dataset/validation", transform=data_transform["val"])  # 测试集数据
