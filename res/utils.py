...
def train_and_val(epochs, model, train_loader, len_train,val_loader, len_val,criterion, optimizer,device):
    ...
    for e in range(epochs):
        ...
        with torch.no_grad():
            ...
            torch.save(model, "./weight0712/last.pth")
            if best_acc <(validation_acc / len_val):
                torch.save(model, "./weight0712/best.pth")
            ...
            writer = SummaryWriter('pest_tf')
            writer.add_scalar('Train_Loss', (running_loss / len_train), e)
            writer.add_scalar('Train_Precision', (training_acc / len_train), e)
            writer.add_scalar('Val_Loss', (val_losses / len_val), e)
            writer.add_scalar('Val_Precision', (validation_acc / len_val), e)
...
def plot_loss(x, history):
    ...
    plt.xlabel('epoch')
    plt.legend(), plt.grid()
    plt.savefig('./weight0712/loss.png')
    plt.show()
...
def plot_acc(x, history):
    ...
    plt.xlabel('epoch')
    plt.legend(), plt.grid()
    plt.savefig('./weight0712/acc.png')
    plt.show()